...
@Autonomous(name = "Odo Right 1+2 Auto", group = "Autonomous")
public class OdoRightSideAutonomous extends LinearOpMode {
    ...
    public void buildTrajectories() {
        ...
        initialDrive = drive.trajectorySequenceBuilder(new Pose2d()) // this does the starting drive to the tall pole and scores the cone
                ...
                .lineToLinearHeading(new Pose2d(-3,20, Math.toRadians(0))) // these two trajectories strafe the robot close to the tall pole tile, avoiding poles and junctions. They have a reduced speed because without them they're too fast and mess with the localization a bit (the localization means the drive encoders)
                .splineToConstantHeading(new Vector2d(-1,44), Math.toRadians(90))
                .lineToLinearHeading(tallPolePose) // drives and turns to the tall pole at a reduced speed because turning fast seems to really mess up the localization
        ...
        getConeOne = drive.trajectorySequenceBuilder(initialDrive.end()) // this drives from the tall pole to the cone stack, grabs a cone, and then drives back to the tall pole and scores it
                ...
                .UNSTABLE_addTemporalMarkerOffset(0.3,()-> startLift(getConeStackHeight(), 0, ROTATE_UPSIDE)) // starts rotating the claw after a delay, avoiding hitting anything with the claw
                .lineToLinearHeading(new Pose2d(0, 51, Math.toRadians(0))) // moves to the center of the tall pole tile to avoid hitting the ground junction and turns to the cone stack at a reduced speed because turning messes with localization
                .lineToLinearHeading(coneStack) // moves to the cone stack at more of a reduced speed because hitting the wall messes with localization and it's not good for the claw
                ...
                .lineToLinearHeading(new Pose2d(8, 51, Math.toRadians(0))) // moves back to the center of the tall pole tile
                .lineToLinearHeading(tallPolePose2) // drives and turns to the tall pole at a reduced speed because it turns and the localization doesn't like high speed turns
                .UNSTABLE_addTemporalMarkerOffset(0,()-> startLift(TALL, ARM_FLIPPED+300, ROTATE_DOWNSIDE)) // dunks the cone on the pole
                .waitSeconds(0.7) // gives the arm time to lower
        ...
        getConeTwo = drive.trajectorySequenceBuilder(getConeOne.end()) // this drives from the tall pole to the cone stack, grabs a second cone, and then drives back to the tall pole and scores it
                ...
                .UNSTABLE_addTemporalMarkerOffset(0.5,()-> startLift(TALL, ARM_FLIPPED-100, ROTATE_DOWNSIDE)) // raises the lift and flips the arm after a short delay to avoid hitting the wall with the arm
                .lineToLinearHeading(new Pose2d(8, 51, Math.toRadians(0))) // moves back to the center of the tall pole tile
                .lineToLinearHeading(tallPolePose3) // drives and turns to the tall pole at a reduced speed because it turns and the localization doesn't like high speed turns
                .UNSTABLE_addTemporalMarkerOffset(0,()-> startLift(TALL, ARM_FLIPPED+300, ROTATE_DOWNSIDE)) // dunks the cone on the pole
                .waitSeconds(0.7) // gives the arm time to lower
                .UNSTABLE_addTemporalMarkerOffset(0,()-> stopLift()) // stops the lift before the end of the trajectory sequence. Again, probably not strictly necessary
                .UNSTABLE_addTemporalMarkerOffset(0,()-> claw.setPosition(CLAW_OPEN)) // drops the cone on the pole
        ...
        parkingOne = drive.trajectorySequenceBuilder(getConeTwo.end()) // this parks in parking space one from the tall pole and resets the arm and lift
                ...
                .lineToLinearHeading(new Pose2d(0, 50, Math.toRadians(-90))) // turns to face the starting wall while moving to the center of the tall pole
                .UNSTABLE_addTemporalMarkerOffset(0,()-> startLift(10, 50, ROTATE_UPSIDE))
                .lineToLinearHeading(new Pose2d(-20, 51, Math.toRadians(-90))) // moves to the first parking zone
        ...
        parkingTwo = drive.trajectorySequenceBuilder(getConeTwo.end()) // this parks in parking space two from the tall pole and resets the arm and lift
                .UNSTABLE_addTemporalMarkerOffset(0,()-> startLift(10, 400, ROTATE_DOWNSIDE)) // starts resetting the arm and lift, but not rotating the claw so that it doesn't hit the cone or pole
                .UNSTABLE_addTemporalMarkerOffset(0.3,()-> startLift(10, 400, ROTATE_UPSIDE)) // starts rotating the claw after a delay, avoiding hitting anything with the claw
                .lineToLinearHeading(new Pose2d(0, 50, Math.toRadians(-90))) // turns to face the starting wall while moving to the center of the tall pole. We don't need to move after this in this trajectory sequence because we're already in parking zone two
                .UNSTABLE_addTemporalMarkerOffset(0,()-> startLift(10, 50, ROTATE_UPSIDE))
        ...
        parkingThree = drive.trajectorySequenceBuilder(getConeTwo.end()) // this parks in parking space three from the tall pole and resets the arm and lift
                ...
                .lineToLinearHeading(new Pose2d(0, 50, Math.toRadians(-90))) // turns to face the starting wall while moving to the center of the tall pole
                .UNSTABLE_addTemporalMarkerOffset(0,()-> startLift(10, 50, ROTATE_UPSIDE))
                .lineToLinearHeading(new Pose2d(21.5, 51, Math.toRadians(-90))) // moves to the third parking zone
